                    ...
                    <?php foreach($mazzi as $value):
                        if($value==="Collezione"):?>
                            <tr class="deck-card">
                                ...
                                <td>
                                    <?php $leader = $conn->query("select c.nome, c.espansione, c.numero from carte c, mazzi m where c.tipo='leader' and m.espansione = c.espansione and m.numero = c.numero and m.codUtente = ".unserialize($_SESSION["user"])->getID()." and m.mazzo = '".$value."' order by uscita")->fetch_assoc();
