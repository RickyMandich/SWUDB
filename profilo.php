                    ...
                    <?php foreach($mazzi as $value):
                        if($value!=="Collezione"):?>
                            <tr class="deck-card">
                                ...
                                <td>
                                    <?php $leader = $GLOBALS["conn"]->query("select c.nome, c.espansione, c.numero from carte c, composizione co, mazzi m where co.codUtente = m.id and c.tipo='leader' and co.espansione = c.espansione and co.numero = c.numero and m.codUtente = ".unserialize($_SESSION["user"])->getID()." and m.nome = '".$value."'")->fetch_assoc();?>
