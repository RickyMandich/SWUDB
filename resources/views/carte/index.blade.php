...
@section('content')
    <div id="row">
        <form action="/carte" class="mb-3 row">
            ...
            <a href="{{ route("carte") }}" class="btn btn-secondary col-1 m-2">Cancella parametri di ricerca</a>
            @foreach ($espansioni as $set)
                <a href="{{ route("carte.set", ["espansione" => $set->espansione]) }}" class="btn btn-secondary col-1 m-2 text-center">
                    {{ $set->espansione }}
                </a>
            @endforeach
        </form>
