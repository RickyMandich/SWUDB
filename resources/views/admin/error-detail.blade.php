...
@section('content')
            ...
            <div class="col-12">
                <div class="d-flex justify-content-between align-items-center mb-4">
                    ...
                    <div class="d-flex gap-2">
                        <button type="button" class="btn btn-outline-info" onclick="copyErrorJson(this)" title="Copia JSON del record errore">
                            <i class="fas fa-copy me-1"></i>Copia JSON Record
                        </button>
                ...
                <div class="row">
                    ...
                    <div class="col-lg-8">
                        ...
                        <div class="card mb-4">
                            ...
                            <div class="card-body">
                                <pre class="bg-custom-light p-3 small"
                                    style="max-height: 400px; overflow-y: auto;"><code>{{ $error->trace }}</code></pre>
                            </div>
                        </div>
                    </div>
                                ...
                                <form method="POST" action="{{ route('admin.errors.update', $error) }}">
                                    ...
                                    <div class="mb-3">
                                        ...
                                        <select name="status" id="status" class="form-select" required>
                                            <option value="new" {{ $error->status === 'new' ? 'selected' : '' }}>Nuovo
                                            </option>
                                            <option value="in_progress" {{ $error->status === 'in_progress' ? 'selected' : '' }}>In Lavorazione</option>
                                            <option value="resolved" {{ $error->status === 'resolved' ? 'selected' : '' }}>
                                                Risolto</option>
                                            <option value="ignored" {{ $error->status === 'ignored' ? 'selected' : '' }}>
                                                Ignorato</option>
                                        </select>
                                    </div>
    ...
    <textarea id="error-json-data" class="d-none">{{ json_encode($error, JSON_PRETTY_PRINT | JSON_UNESCAPED_UNICODE | JSON_UNESCAPED_SLASHES) }}</textarea>

    <script>
    function copyErrorJson(button) {
        const jsonText = document.getElementById('error-json-data').value;
        
        if (navigator.clipboard && window.isSecureContext) {
            navigator.clipboard.writeText(jsonText)
                .then(() => showCopySuccess(button))
                .catch(() => fallbackCopy(jsonText, button));
        } else {
            fallbackCopy(jsonText, button);
        }
    }

    function fallbackCopy(text, button) {
        const textarea = document.getElementById('error-json-data');
        ...
        try {
            ...
            alert('Errore durante la copia del JSON');
        } finally {
